...
namespace Roulette.Tests.Services
{
    public class BetServiceTests
    {
        ...
        [Fact]
        public async Task AddBet_ReturnsBet()
        {
            ...
            result.Bet.Should().Be(expectedBet);
        }
        ...
        public async Task UpdateBet_ReturnsBet()
        {
            ...
            mockGameRepository.GetById(Arg.Any<Guid>()).Returns(new Game());
            var mockBetRepository = Substitute.For<IBetRepository>();
            mockBetRepository.UpdateBet(Arg.Any<Guid>(), Arg.Any<double>())
                .Returns(expectedBet);
            ...
            var result = await service.UpdateBet(Guid.NewGuid(), Guid.NewGuid(), new UpdateBetRequest());

            result.Should().BeOfType(typeof(UpdateBetResponse));
            result.Bet.Should().Be(expectedBet);
        }
